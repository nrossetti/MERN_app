...
import { createProject, updateProject } from '../../actions/posts';

const Form = ({ currentId, setCurrentId }) => {
  const [postData, setPostData] = useState({ title: '', author: '', description: '', tags: '', selectedFile: '' });
  ...
  const clear = () => {
    setCurrentId(0);
    setPostData({ title: '', author: '', description: '', tags: '', selectedFile: '' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentId === 0) {
      ...
      dispatch(updateProject(currentId, postData));
      clear();
    }
  };
  ...
  return (
    <Paper className={classes.paper}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${post.title}"` : 'Post a New Project'}</Typography>
